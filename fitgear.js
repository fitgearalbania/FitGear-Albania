/* Zbret sasinë e blerë nga stoku — përdoret kur dikush bën porosi */
function doPost(e) {
  ...
  try {
    ...
    const items = JSON.parse(e.postData.contents); // [{id, variant, qty}, ...]

    items.forEach(item => {
      for (let i = 1; i < data.length; i++) {
        const rowId = String(data[i][0]);
        const rowVariant = String(data[i][1] || '');
        if (rowId === item.id && rowVariant === (item.variant || '')) {
          const current = Number(data[i][2]) || 0;
          const updated = Math.max(0, current - Number(item.qty || 0));
...
/* ====================================================================
   LIDHJA ME GOOGLE SHEETS — vendos këtu URL-në e Web App që merr
...
/* ====================================================================
   STOKU LIVE NGA GOOGLE SHEETS
   ==================================================================== */
function buildStockKey(id, variant){
  return id + '||' + (variant || '');
}

/* Merr numrat e lexuar nga Sheet-i dhe i vendos mbi PRODUCTS/OFFERS */
function applyStockMap(list, stockMap){
  list.forEach(item=>{
    if(Array.isArray(item.colors)){
      item.colors.forEach(color=>{
        if(color.sizes){
          color.sizes.forEach(size=>{
            const k = buildStockKey(item.id, color.name + ' / ' + size);
            if(stockMap[k] !== undefined){
              if(!color.sizeStock) color.sizeStock = {};
              color.sizeStock[size] = stockMap[k];
            }
          });
          color.inStock = color.sizeStock ? Object.values(color.sizeStock).some(v=>v>0) : color.inStock;
        } else {
          const k = buildStockKey(item.id, color.name);
          if(stockMap[k] !== undefined){
            color.stock = stockMap[k];
            color.inStock = stockMap[k] > 0;
          }
        }
      });
    }
    if(Array.isArray(item.options)){
      item.options.forEach(option=>{
        const k = buildStockKey(item.id, option.label);
        if(stockMap[k] !== undefined){
          option.stock = stockMap[k];
          option.inStock = stockMap[k] > 0;
        }
      });
    }
    if(!Array.isArray(item.colors) && !Array.isArray(item.options)){
      const k = buildStockKey(item.id, '');
      if(stockMap[k] !== undefined){
        item.stock = stockMap[k];
        item.inStock = stockMap[k] > 0;
      }
    }
  });
}
...
async function loadStockFromSheet(){
  ...
  try{
    ...
    const stockMap = {};
    rows.forEach(r=>{ stockMap[buildStockKey(r.id, r.variant)] = Number(r.stock); });
    applyStockMap(PRODUCTS, stockMap);
    applyStockMap(OFFERS, stockMap);
  }catch(err){
    console.warn('Nuk u lexua dot stoku nga Google Sheets, po përdoren numrat nga kodi.', err);
  }
}
...
async function reserveStockOnSheet(cartItems){
  ...
  try{
    ...
    console.warn('Nuk u përditësua dot stoku në Google Sheets.', err);
  }
}
...
checkoutForm.addEventListener('submit', async function(e){
  ...
  try{
    ...
    if(res.ok){
      ...
      await reserveStockOnSheet(cart);
      renderProductGrid();
      renderOffers();
...
/* Përditësim optimist lokal — që faqja të duket e saktë menjëherë,
   ndërkohë që reserveStockOnSheet() e ruan ndryshimin përgjithmonë
   te Google Sheets, i dukshëm për të gjithë vizitorët. */
...
async function initStore(){
  await loadStockFromSheet(); // lexon stokun live nga Google Sheets
  renderProductGrid();
  renderOffers();
  renderCart();
}
